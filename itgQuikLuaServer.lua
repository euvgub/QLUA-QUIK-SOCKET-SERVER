...
if appDataFolder ~= nil then
  owLogsPath = appDataFolder .. "\\Option Workshop\\Logs\\"
  iniPath = appDataFolder .. "\\Option Workshop\\transport\\quik_lua.ini"
...
local pcallStatus, config = pcall(LIP.load(iniPath))
...
if config == nil then
  trace('info', 'Ini file not found')
  transport.config = {}
  transport.config.dataflow = {}
  transport.config.dataflow.receive_orders = true
else
  trace('info', 'Ini file found')
